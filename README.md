...
Проект учебный, поэтому в нём сознательно упрощены несколько вещей:

- Нет авторизации и персональных аккаунтов.
- Нет сохранения корзины между сессиями (localStorage).
- Нет тестов (unit/e2e).

В качестве дальнейших доработок я бы добавила:
- unit-тесты для `AppState` и валидации форм;
- сохранение корзины и заказа в `localStorage`;
- обработку сетевых ошибок с повтором запросов и пользовательскими уведомлениями.
